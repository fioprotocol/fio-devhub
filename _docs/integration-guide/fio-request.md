...
# Using FIO Requests for deposits and withdrawals on exchanges
...
* BTC public address, amount, memo and other data [are encrypted]({{site.baseurl}}/docs/general-functions/encryption). If you are using the FIO SDK, encryption is done as part of the `requestFunds` function call.
* [newfundsreq]({{site.baseurl}}/pages/api/fio-api/#options-newfundsreq) is submitted to FIO Chain.
* [/get_sent_fio_requests]({{site.baseurl}}/pages/api/fio-api/#post-/get_sent_fio_requests) will return the request just sent with the status requested and all encrypted data including “Invoice 123” memo. We recommend wallets show this request with a status of “pending”.
* [/get_pending_fio_requests]({{site.baseurl}}/pages/api/fio-api/#post-/get_pending_fio_requests) will not return anything as this request was for Bob, not for Alice.

#### Bob checks his wallet for new requests

* [/get_sent_fio_requests]({{site.baseurl}}/pages/api/fio-api/#post-/get_sent_fio_requests) will not return anything as this request was sent by Alice not by Bob.
* [/get_pending_fio_requests]({{site.baseurl}}/pages/api/fio-api/#post-/get_pending_fio_requests) will return the request just sent with the status requested and all encrypted data including “Invoice 123” memo. We recommend wallets show this request for Bob to act on.
...
* [recordobt]({{site.baseurl}}/pages/api/fio-api/#options-recordobt) is sent to FIO Chain with `status` set to `sent_to_blockchain`.

#### Alice checks payment

* [/get_sent_fio_requests]({{site.baseurl}}/pages/api/fio-api/#post-/get_sent_fio_requests) will return the request with the status `sent_to_blockchain`. We recommend wallets show this request as “received”.
...
* [/get_sent_fio_requests]({{site.baseurl}}/pages/api/fio-api/#post-/get_sent_fio_requests) will not return anything as Bob never sent a request.
* [/get_pending_fio_requests]({{site.baseurl}}/pages/api/fio-api/#post-/get_pending_fio_requests) will not return anything as Bob already responded to Alice’s request.
* [/get_received_fio_requests]({{site.baseurl}}/pages/api/fio-api/#post-/get_received_fio_requests) will return the the request with the status sent_to_blockchain. We recommend wallets show this request as “paid”.
...
| Updating Request status | When crypto is sent in Response to a FIO Request, the request status should be updated to `sent_to_blockchain` using [recordobt]({{site.baseurl}}/pages/api/fio-api/#options-recordobt) and optional metadata such as amount, currency, and memo, may be recorded on the FIO Chain. This FIO Data is encrypted and only readable by Payee and Payer. |
